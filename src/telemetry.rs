...
//! Telemetry module
//!
//! This module encompasses telemetry for `archon`.
...
pub fn register_shutdown() {
    ctrlc::set_handler(move || {
        println!();
        tracing::info!(target: "archon", "shutting down...");
        std::process::exit(0);
    })
...
pub fn init(verbose: bool) -> Result<()> {
    let subscriber = match verbose {
        true => get_subscriber("archon=debug".into()),
        false => get_subscriber("archon=info".into()),
    };
